...
| `filesystem.read.v1` | Hardened inline + streaming slice | Grants bind either a user-approved logical path or a typed standard directory resolved from trusted host session state. Every operation reopens that root with symlink-free `openat2` and resolves plugin-relative paths with `RESOLVE_BENEATH`, `RESOLVE_NO_SYMLINKS`, `RESOLVE_NO_MAGICLINKS`, and `RESOLVE_NO_XDEV`. Absolute/parent paths, intermediate and final symlinks, nested mounts, special files, multi-link files, oversized files, and enumeration floods fail closed. Legitimate root replacement preserves path-scoped consent; each in-flight read uses one pinned descriptor with bounded backpressure and cancellation/revocation. |
...
| `filesystem.write.v1` | Hardened inline + streaming slice | Exact create/replace/append/delete/rename/create-directory bits operate beneath logical path or typed standard-directory roots reopened with symlink-free `openat2`; symlinks and nested mount crossings fail. Parent directories and existing files are descriptor-pinned for each in-flight operation; create/replace/append stage and atomically commit complete content, rename never overwrites, delete quarantines and inode-checks before unlink, regular files must have one link, plugin mutations are serialized, and broker-private random names cannot be requested. Large writes use exact-offset broker resources with complete-size commit and abort/revocation cleanup. Payload/file caps and a durable source-bound rolling-hour ledger are enforced before I/O. |
...
| `appearance.provide.v1` | Implemented sandboxed worker slice | A grant names exact package-local provider IDs, installer-bound roots, and file-size/update-rate ceilings. Component-world projection gives each separate provider only its declared ID and mounts; its supervisor registers only bounded `read-file` and typed `publish`, while renderer components neither receive nor depend on that authority. The authenticated supervisor validates identity and rate before forwarding semantic colors. The session daemon has no provider-specific paths or parsers and owns only provider arbitration, derived roles, accessibility policy, and atomic generations. Invalid or incomplete publications never displace the last valid snapshot. |
