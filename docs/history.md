...
This is the development log that used to be the project README. It records what
was built in what order, and the acceptance measurements taken on real hardware
at each step. It is kept because those numbers are evidence, not because it is a
good introduction — start at the [README](../README.md) for that.

Commands and paths below are historical and may predate later renames.
